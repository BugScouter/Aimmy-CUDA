...
namespace Aimmy2.AILogic
{
    public class ModelManager
    {
        ...
        public bool IsLegacyV8 { get; set; } = true;
        ...
        public async Task LoadModelAsync(string modelPath, int IMAGE_SIZE, bool failure = false) // default value for failure is false, obviously
        {
            ...
            {
                if (ex.Message.Contains("TensorRT") && ex.Message.Contains("cache"))
                {
                    Log(LogLevel.Warning, "TensorRT shape mismatch detected from previous cached engine. Re-initializing cache...");
                    try
                    {
                        if (Directory.Exists("bin/tensorrt_cache"))
                        {
                            Directory.Delete("bin/tensorrt_cache", true);
                        }
                        Directory.CreateDirectory("bin/tensorrt_cache");
                    }
                    catch (Exception) { }
                    
                    // Recursive re-load after cache clear
                    await LoadModelAsync(modelPath, IMAGE_SIZE, failure);
                    return;
                }
        ...
        public bool ValidateOnnxShape(int IMAGE_SIZE)
        {
            if (onnxModel != null)
            {
                ...
                else
                {
                    ...
                    var expectedShapeV8 = new int[] { 1, 4 + NUM_CLASSES, NUM_DETECTIONS };
                    var expectedShapeYolo26 = new int[] { 1, 300, 6 };
                    
                    if (outputMetadata.Values.All(metadata => metadata.Dimensions.SequenceEqual(expectedShapeV8)))
                    {
                        IsLegacyV8 = true;
                    }
                    else if (outputMetadata.Values.Any(metadata => metadata.Dimensions.SequenceEqual(expectedShapeYolo26)))
                    {
                        IsLegacyV8 = false;
                        NUM_DETECTIONS = 300;
                        NUM_CLASSES = 1;
                    ...
                    {
                        Log(LogLevel.Error,
                            $"Output shape does not match the expected shape of {string.Join("x", expectedShapeV8)} or YOLO26. Model may not work.",
                            true, 10000);
                        return false;
                    }
